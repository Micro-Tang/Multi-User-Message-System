...
if (@$_GET['action'] == 'send') {
    ...
    $message_array['flower'] = mysqli_real_escape_string($conn, $_POST['flower']);
        //插入数据库
    _query("insert into tg_flower(tg_touser, tg_fromuser, tg_content, tg_flower, tg_date)
                                   VALUES 
                                   ('{$message_array['touser']}', 
                                   '{$message_array['fromuser']}',
                                   '{$message_array['content']}',
                                   '{$message_array['flower']}',
                                   now())");
                ...
                <select name="flower">
                    <?php
                        foreach(range(1, 100) as $number) {
                            echo '<option value="'.$number.'"> x '.$number.' 朵🌹</option>';
                        }
                    ?>
                </select>
            </dd>
            <dd>
                <textarea name="content">"<?php echo $_COOKIE['username'] ?>": 赠人玫瑰,手有余香。</textarea>
            </dd>
            <dd>
                ...
                <input type="submit" class="submit" name="submit" value="送花" />
            </dd>
        </dl>
    </form>
</div>
